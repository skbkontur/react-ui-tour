...
const withStep = (id: string) => ({ onNext, onPrev, onClose }) => (
  <div id={id}>
  ...
  render() {
    return (
      <div>
        ...
        <button className="run" onClick={this.run}>
          run
        </button>
      </div>
    );
...
describe("processMove. delays", () => {
  ...
  const callbacks = new Array<jest.Mock>(4).fill(null).map(() => jest.fn());
  const [before1, before2, after1, after2] = callbacks;
  const move = jest.fn();
  const clear = jest.fn();
  const [onBefore1, onBefore2, onAfter1, onAfter2] = callbacks.map(cb => () =>
    delay(cb)
  );
  ...
  it("nothing should be called except clear", () => {
    expect(callbacks.reduce((acc, fn) => acc + fn.mock.calls.length, 0)).toBe(
      0
    );
...
describe("processMove. group", () => {
  ...
  let clear;
  beforeEach(() => {
    const callbacks = new Array<jest.Mock>(4)
      .fill(null)
      .map(() => jest.fn(() => Promise.resolve()));
  ...
  it("onBefore & onAfter should be called in different groups", () => {
    ...
    return processMove(step1, step2, move, clear)
      .then(() => {
        expect(move).toBeCalled();
        expect(stepProps1.onAfter).toBeCalled();
        expect(stepProps2.onBefore).toBeCalled();
